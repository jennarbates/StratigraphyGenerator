...
Open the local app and choose the page that asks you to add a trench drawing. You will need one supported file and a clear idea of whether the sheet is an illustrated trench sheet or a hand-drawn field sheet. The screen also offers a "Create a diagram from scratch" option that opens a blank drawing canvas instead of an upload; this page covers the upload path.
...
1. Input: the sketch type you want to trace.
   - Action: pick the sheet type before you upload, because the choice is sent with the file. Choose an illustrated trench sheet for a more polished drawing with layer labels, or a hand-drawn field sheet for a graph-paper style drawing with locus numbers. If the sheet belongs to a trench you plan to combine later, also fill in the optional trench, wall, season, and site grid fields.
   - Artifact: the selected sheet type and any labels are ready to be sent with the upload.
2. Input: a supported drawing file and the local app.
   ...
   - Artifact: the file is saved to the job's scan folder, the preview area is populated, and the job metadata records the sheet type and the scan path.
...
- Job metadata that records the sheet type, the file name, the scan path, and any optional trench, wall, season, or site grid labels you supplied.
