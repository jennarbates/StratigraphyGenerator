"""Pixel-space tracing converted to a complete extraction document.

The browser supplies calibration clicks, boundary polylines and optional
feature polygons; this module turns them into the same schema the Gemini
extractors emit, deterministically and with no model involved.

This was the bulk of backend/routes/manual.py. It is domain logic, not an HTTP
concern: it holds no Flask imports and can be unit-tested without an app, which
is how tests/test_manual_routes.py and tests/test_locus_top_boundaries.py had
been using it all along -- by importing a route module to get at it.
"""
...
def make_calibration(payload: dict[str, Any]) -> Calibration:
