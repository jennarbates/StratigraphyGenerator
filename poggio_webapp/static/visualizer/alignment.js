...
let ALIGN = {x:0, y:0, w:1, h:1};
function alignKey(){ return "viz_align_" + (new URLSearchParams(location.search).get("job") || "manual"); }
try{ const saved = localStorage.getItem(alignKey());
     if(saved) ALIGN = JSON.parse(saved); }catch(e){}
// a corrupt or sliver-thin saved box would make the overlay invisible with no
// hint why — snap back to full-image and forget it
if(!(ALIGN.w>0.02 && ALIGN.h>0.02 && ALIGN.x>=0 && ALIGN.y>=0 &&
     ALIGN.x+ALIGN.w<=1.001 && ALIGN.y+ALIGN.h<=1.001)){
