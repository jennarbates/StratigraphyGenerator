...
def _validate_face_names(faces: list[dict]) -> None:
    ...
    for face_index, face in enumerate(faces):
        if not isinstance(face, dict):
            raise EditorStateStructureError(f"Face {face_index + 1} must be an object.")
        ...
        if normalized_name in normalized_names:
            raise DuplicateFaceNameError(
                f'Duplicate face name "{face_name.strip()}"; face names must be unique.'
            )
...
def _validate_polygons(
    ...
    resume_faces = resume_state.get("faces") if isinstance(resume_state, dict) else None
        ...
        for polygon in polygons:
            ...
            if schema_type == "FieldWallProfile":
                if not _has_required_text(metadata.get("locus")):
                    raise PolygonMetadataError(
                        f'Face "{face_name}" polygon {polygon_id} needs a locus.'
                    )
                ...
                raise PolygonMetadataError(
                    f'Face "{face_name}" polygon {polygon_id} needs a material.'
                )
...
def _validate_grid_registration(
    ...
    registered_faces = (
        grid_config.get("faces", {}) if isinstance(grid_config, dict) else {}
    )
...
def _validate_editor_structure(
    ...
    if not isinstance(editor_state, dict):
        raise EditorStateStructureError(
            "Assembled editor state must include an editorState structural snapshot."
        )
