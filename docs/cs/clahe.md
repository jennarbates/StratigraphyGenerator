...
`high_contrast()` does **not** use CLAHE. It goes straight from
...
| **Linear or percentile contrast stretch** | Map the 1st–99th percentile to full range | Predictable, cheap, no noise amplification, and global: the same objection as equalisation. Percentile clipping does make it robust to specks, which plain min/max is not. |
...
plus a bilinear blend per pixel. Memory is 64 histograms of 256 bins, which is
negligible.
