...
no shared x stations at all, so the two cannot be compared
...
An **error**, not a warning, because layers cannot cross. Compare the adjacent check on
...
| **Fit a smooth curve** ([spline](linear-interpolation.md)) | A differentiable boundary | Overshoots, claiming excursions nobody recorded, which is the opposite of what a [fabrication-detecting](coefficient-of-variation.md) validator should do. |
...
rather than necessary, since the callers generally supply sorted points. It is cheap
