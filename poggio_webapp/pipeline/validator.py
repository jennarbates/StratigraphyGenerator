...
def check_parallel_layers(layers, where, report):
    ...
    for layer in layers or []:
        ...
        if len(pts) >= 4:
            name = (
                layer.get("inferredMaterial")
                or layer.get("layerName")
                or layer.get("locusNumber")
                or "?"
            )
        ...
        if has_shape and top_min is not None and bot_max is not None:
            for pi, p in enumerate(sp):
                ...
                if y is None:
                    continue
                if (
                    y < top_min - monotonic_tolerance_m
                    or y > bot_max + monotonic_tolerance_m
                ):
                    report.warn(
                        f"{fwhere}[{pi}]",
...
def check_face(
    ...
    for li, layer in enumerate(layers):
        ...
        top = check_boundary(
            layer.get("topBoundary"), f"{where} top", report, max_plausible_depth_m
        )
        bottom = check_boundary(
            layer.get("bottomBoundary"),
            f"{where} bottom",
            report,
            max_plausible_depth_m,
        )
        ...
        if source != "manual_editor":
            check_uniform_spacing(
                layer.get("bottomBoundary"), f"{where} bottom", report
            )
...
def _check_field_wall_extras(data, report):
    ...
    layer_nums = {
        str(layer.get("locusNumber", "")).strip()
    ...
    for num in sorted(layer_nums - set(locus_nums)):
        if num:
            report.warn(
                where,
                f"layer references locus {num}, which has no "
...
def validate(
    data,
    monotonic_tolerance_m=DEFAULT_MONOTONIC_TOLERANCE_M,
...
def run_validate(
    input_path: str,
    monotonic_tolerance=DEFAULT_MONOTONIC_TOLERANCE_M,
