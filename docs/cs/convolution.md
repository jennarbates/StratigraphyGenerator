...
| **Learned filters (a CNN)** | Train convolution weights from data instead of choosing them | The same operation with weights fitted rather than designed. It needs labelled data this project does not have, and it produces a filter nobody can explain — against the grain of a pipeline whose value is that every step is inspectable. |
