...
is disabled. `single_face_note` is either `null` or the builder's warning that
a surface constrained by one face is interpolated across the full extent.
...
renderer. Phase A renders boundary surfaces only; it does not render
`trench_model_lith_block.npz` as a solid lithology volume.
